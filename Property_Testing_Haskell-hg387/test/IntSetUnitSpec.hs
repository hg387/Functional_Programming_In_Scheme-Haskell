...
spec = do
  ...
  describe "merge" $ do
    it "merge test for first empty IntSet" $
        merge (IntSet []) (IntSet [(1,5),(8,10),(11,15)]) `shouldBe` IntSet [(1,5),(8,10),(11,15)]
    it "merge test for second empty IntSet" $
        merge (IntSet [(1,5),(8,10),(11,15)]) (IntSet []) `shouldBe` IntSet [(1,5),(8,10),(11,15)]
    it "merge test for duplicate IntSet pairs" $
        merge (IntSet [(1,5),(6,6),(8,10)]) (IntSet [(1,5),(6,6),(8,10)]) `shouldBe` IntSet [(1,5),(6,6),(8,10)]
    it "merge test for small IntSet pairs" $
        merge (IntSet [(1,5),(8,10)]) (IntSet [(6,7),(9,10)]) `shouldBe` IntSet [(1,5),(6,7),(8,10)]
    it "merge test for IntSet pairs contaning single element intervals" $
        merge (IntSet [(1,5),(6,6),(8,10)]) (IntSet [(6,7),(8,8),(9,10),(11,15)]) `shouldBe` IntSet [(1,5),(6,7),(8,10),(11,15)]
    it "merge test for IntSet pairs contaning more than five intervals each with mix intervals" $
        merge (IntSet [(1,5),(6,6),(8,10),(11,12),(20,25),(26,26),(30,37)]) (IntSet [(6,7),(8,8),(9,10),(11,15),(16,20),(21,25),(28,28)]) `shouldBe` IntSet [(1,5),(6,7),(8,10),(11,15),(16,25),(26,26),(28,28),(30,37)]
  ...
  describe "insert" $ do
    ...
    it "inserting into set at first location" $
      insert (-1) (IntSet [(0,1),(5,7)]) `shouldBe` IntSet [(-1,-1),(0,1),(5,7)]
    it "inserting into set at last location" $
      insert 14 (IntSet [(0,1),(3,5),(6,6),(7,9),(10,13),(14,16)]) `shouldBe` IntSet [(0,1),(3,5),(6,6),(7,9),(10,13),(14,16)]
    it "inserting into set at mid location" $
      insert 7 (IntSet [(0,1),(3,5),(6,6),(8,9),(10,13),(14,16)]) `shouldBe` IntSet [(0,1),(3,5),(6,6),(7,7),(8,9),(10,13),(14,16)]  
